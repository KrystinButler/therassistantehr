...
exports.createSupabasePaymentRepository = createSupabasePaymentRepository;
const helpers_1 = require("./helpers");
function createSupabasePaymentRepository(db) {
    return {
        ...
        async listUnpostedPayments(organization_id) {
            const rows = await (0, helpers_1.expectMany)(db
          ...
          `)
            ...
            return rows.map((row) => ({
                ...
                patient_name: [row.client?.first_name, row.client?.last_name].filter(Boolean).join(" ").trim() ||
                    null,
