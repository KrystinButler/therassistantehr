...
exports.createSupabaseScheduleRepository = createSupabaseScheduleRepository;
const helpers_1 = require("./helpers");
function createSupabaseScheduleRepository(db) {
    return {
        async getScheduleDay(request) {
            const { start, end } = (0, helpers_1.buildScheduleDateRange)(request.date);
            let query = db
                .from("appointments")
                .select(`
          id,
        ...
        `, { count: "exact" })
            ...
            const encounters = appointmentIds.length
                ? await (0, helpers_1.expectMany)(db
                    .from("encounters")
                    .select(`
                id,
                organization_id,
            ...
            const claims = encounterIds.length
                ? await (0, helpers_1.expectMany)(db
                    ...
                    .order("created_at", { ascending: false }))
                : [];
            const claimByEncounterId = new Map();
            ...
            const rows = (data ?? []).map((row) => {
                ...
                const providerFullName = row.provider?.display_name ||
        ...
        async createEncounterForAppointment(args) {
            ...
            const created = await (0, helpers_1.expectOne)(db
                .from("encounters")
                .insert({
        ...
        async setAppointmentEncounterIdIfColumnExists(organization_id, appointment_id, encounter_id) {
            ...
            if (error.message.includes("encounter_id") &&
